...
class SubscriptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $invoice = Invoice::create([
            ...
            'status' => 'unpaid',
            'subtotal_cents' => $subscription->price * 100,
            'total_cents' => $subscription->price * 100,
            'currency' => 'USD',
            'due_date' => $subscription->next_due_date ?? now()->addDays(7),
        ]);

        $invoice->items()->create([
            'item_type' => 'subscription',
            'reference_id' => $subscription->id,
            'description' => 'خطة اشتراك #' . $subscription->plan_id,
            'qty' => 1,
            'unit_price_cents' => $subscription->price * 100,
            'total_cents' => $subscription->price * 100,
        ]);
    ...
    public function update(Request $request, Subscription $subscription)
    {
        ...
        // تعديل الفاتورة المرتبطة (إن وجدت)
        $invoiceItem = $subscription->invoiceItems()->first();

        if ($invoiceItem && $invoiceItem->invoice) {
            $itemTotal = $data['price'] * 100;

            $invoiceItem->update([
                'unit_price_cents' => $itemTotal,
