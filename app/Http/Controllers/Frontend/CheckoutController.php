...
class CheckoutController extends Controller
{
    ...
    public function process(Request $request, $template_id)
    {
        // dd($request->all());
        {
            ...
            $order = null;
            if (auth('client')->check()) {
                // Normalize domain_option values coming from frontend to the values used by DB/app
                $rawOption = $request->input('domain_option') ?? 'own';
                $optionMap = [
                    'register' => 'new',
                    'subdomain' => 'subdomain',
                    'own' => 'existing',
                ...
                $normalizedOption = $optionMap[$rawOption] ?? $rawOption;
            ...
            if ($order) {
                ...
                // إنشاء اشتراك وربطه بالطلب إن وُجدت خطة مرتبطة بالقالب
                try {
                    $plan = $template?->plan;
                    if ($plan) {
                        \App\Models\Subscription::create([
                            ...
                            'price' => $showDiscount ? $discPrice : $basePrice,
                    ...
                    // لا نوقف العملية الرئيسية عند فشل إنشاء الاشتراك، لكن يمكن تسجيل الخطأ لاحقاً
                }
            }
