@php
    ...
    $ctaButtonLabel = trim((string) ($primaryButton['label'] ?? 'ابدأ الآن — موقعك جاهز خلال دقائق'));
    $ctaButtonUrl = trim((string) ($primaryButton['url'] ?? '#'));
    $ctaButtonVisible = filter_var($primaryButton['visible'] ?? true, FILTER_VALIDATE_BOOLEAN);
    $ctaButtonNewTab = filter_var($primaryButton['new_tab'] ?? true, FILTER_VALIDATE_BOOLEAN);
...
<section id="how-we-build"
    class="{{ $paddingY }} overflow-hidden bg-purple-brand px-4 text-center text-white sm:px-6 lg:px-12">
    <div class="container mx-auto">
        ...
        @if ($steps->isNotEmpty())
            <div
                class="relative mx-auto flex max-w-7xl flex-col items-center gap-12 md:flex-row md:flex-wrap md:justify-center md:gap-10">
                @foreach ($steps as $step)
                    @php
                        $isAccent = $step['is_accent'];
                        $cardClasses = $isAccent ? 'bg-red-brand text-white z-20' : 'bg-white text-purple-brand z-10';
                        $iconClasses = $isAccent ? 'text-white' : 'text-red-brand';
                    @endphp

                    <div
                        class="relative flex min-h-[7.5rem] w-full flex-col items-center justify-center gap-3 rounded-2xl p-6 shadow-lg transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl md:w-[17rem] md:flex-none {{ $cardClasses }}">
                        @if (($step['icon_source'] ?? 'class') === 'svg' && !empty($step['icon_svg']))
                            <span class="inline-flex h-7 w-7 items-center justify-center {{ $iconClasses }}"
                                aria-hidden="true">
                                {!! $step['icon_svg'] !!}
                            </span>
                        ...
                        @if (!$loop->last)
                            <div
                                class="pointer-events-none absolute bottom-[-1rem] left-1/2 h-8 w-8 -translate-x-1/2 rotate-45 transform md:bottom-auto md:top-1/2 md:-translate-y-1/2 ltr:md:left-auto ltr:md:right-[-1rem] ltr:md:translate-x-0 rtl:md:left-[-1rem] rtl:md:right-auto rtl:md:translate-x-0 {{ $isAccent ? 'bg-red-brand' : 'bg-white' }}">
                            </div>
                        @endif
                    </div>
                @endforeach
    ...
    <!-- CTA -->
    @if ($ctaButtonVisible && $ctaButtonLabel !== '' && $ctaButtonUrl !== '')
        <div class="mt-12">
            <a href="{{ $ctaButtonUrl }}" @if ($ctaButtonNewTab) target="_blank" rel="noopener" @endif
