...
**الحالة:** تحليل بدون تعديلات برمجية (Read-Only Architectural Review)
...
├─────────────────────────────────────────────────────────────────┤
│  Layer 4: Auto Blade Generator (Phase 6)                         │
...
ComponentLibrary::COMPONENT_FIELD_GROUPS → يُحدد انتماء كل field_key لـ component
```
...
```php
// خريطة عكسية: field_key → component name
...
// detectComponentGroups() يُنشئ الخريطة العكسية:
// 'eyebrow' → 'intro', 'title' → 'intro', 'button_url' → 'cta', ...
// ثم يجمّع الحقول بحسب component بالترتيب الـ canonical
```
...
**٣. component_names في BladeGenerator غير مُتزامنة مع ComponentLibrary**  
`COMPONENT_FIELD_GROUPS` في `BladeGenerator` هي نسخة مكررة من `ComponentLibrary`. إذا أضفنا Component جديد في ComponentLibrary، يجب تحديث BladeGenerator يدوياً.
...
### ١. Phase 2 — Auto Write Blade File (تأثير: عالي، سهولة: عالية)

**المشكلة:** المطور حالياً يجب أن يضغط "كتابة الملف" يدوياً بعد توليد الـ scaffold.

**المقترح:** إضافة زر "⚡ توليد + كتابة" يجمع BladeGenerator + SectionTemplateFileWriter في خطوة واحدة:
...
if ($request->boolean('write_to_disk')) {
    ...
    $result = app(SectionTemplateFileWriter::class)->write($sectionDefinition);
}
```

**العائد:** يُقلص وقت "من تعريف إلى Blade قابل للاستخدام" من ~2 دقيقة إلى أقل من 10 ثوانٍ.
...
### ٥. ComponentLibrary ↔ BladeGenerator Sync (تأثير: متوسط، سهولة: عالية)

**المشكلة:** `COMPONENT_FIELD_GROUPS` في `BladeGenerator` هي نسخة مكررة من `ComponentLibrary`. إذا أضفنا Component جديد، يجب تحديث ملفَين.

**المقترح:** `BladeGenerator` يستخدم `ComponentLibrary::all()` مباشرة:

```php
// بدلاً من COMPONENT_FIELD_GROUPS ثابت في BladeGenerator:
private function buildComponentFieldMap(): array
{
    $map = [];
    foreach (ComponentLibrary::all() as $componentKey => $component) {
        foreach ($component['fields'] as $field) {
            $map[$field['field_key']] = $componentKey;
        }
    }
    return $map;
}
...
**العائد:** Single Source of Truth — إضافة Component في مكان واحد فقط يُؤثر على كل النظام.
...
1. Phase 2 (Auto Write Blade) ← أكبر تأثير بأقل جهد
2. ComponentLibrary ↔ BladeGenerator Sync ← تقني بسيط يمنع bugs مستقبلية

**الأولوية المتوسطة المدى:**
3. Section Rendering Cache ← ضروري عند scale
4. BladeGenerator Design Awareness ← يحسن جودة الـ scaffold

**الأولوية طويلة المدى:**
5. Database-backed Templates ← Marketplace readiness

---

*آخر تحديث: 2026-06-19 | مُراجَع مع: SectionRenderer, SectionDefinitionFrontendViewDataFactory, SectionDefinitionRuntimeResolver, SectionTemplateRegistry, ComponentLibrary, SectionTemplateLibrary, FieldPresetLibrary, BladeGenerator, SectionTemplateFileWriter, SectionDefinition Model, SectionDefinitionField Model*
