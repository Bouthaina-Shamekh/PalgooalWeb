  ...
  <main class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-0 sm:px-12" data-aos="zoom-in" data-aos-delay="200">
    @forelse ($templates as $template)
      ...
      <a href="/templates/{{ $translation->slug }}" class="block h-full">
        <article class="h-full flex flex-col bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden relative group transition-transform duration-300 hover:-translate-y-2 hover:shadow-2xl border border-primary/10">
          ...
          <div class="p-5 flex flex-col justify-between flex-grow rtl:text-right ltr:text-left">
            <div>
              <h3 class="text-suptitle font-bold text-primary/90 dark:text-white group-hover:text-secondary mb-2">{{ $translation->name }}</h3>
              <p class="text-primary/70 dark:text-gray-300 text-sm">{{ Str::limit(strip_tags($translation->description), 70) }}</p>
            </div>
            <div class="mt-4 flex justify-between items-center text-sm font-bold">
              ...
              <div class="flex items-center gap-2">
                @if ($template->discount_price)
                  ...
                  <span class="text-title-h3 text-secondary dark:text-yellow-400">${{ $template->discount_price }}</span>
                @else
                  <span class="text-title-h3 text-secondary dark:text-yellow-400">${{ $template->price }}</span>
                @endif
              </div>
              <div>
