...
<section class="dark:bg-gray-900 text-primary dark:text-white py-16 px-4 sm:px-8 lg:px-24 rtl:text-right ltr:text-left" aria-labelledby="why-us-heading">
    <header class="text-center mb-12">
        <h2 id="why-us-heading" class="text-title-h2 font-extrabold animate-fade-in-up" data-aos="fade-up"
            data-aos-delay="200">
            {{ $data['title'] ?? 'عنوان غير متوفر' }}
        </h2>
        <p class="text-tertiary mt-4 text-suptitle max-w-max mx-auto animate-fade-in-up delay-100" data-aos="fade-up"
            data-aos-delay="200">
            {{ $data['subtitle'] ?? 'وصف غير متوفر' }}
        </p>
    </header>

    <main class="relative group" aria-label="{{ $data['title'] ?? 'عنوان غير متوفر' }}" role="region" data-aos="zoom-in" data-aos-delay="200">
        <div class="swiper" id="clients-swiper">
            @php
                $works = App\Models\Portfolio::paginate(6);
            @endphp
