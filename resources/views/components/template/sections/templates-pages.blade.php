            ...
            <aside class="space-y-8">
                ...
                <div class="border border-gray-200 dark:border-gray-700 p-4 rounded-xl shadow-md">
                    ...
                    <ul class="space-y-2 text-gray-800" id="categoryFilter">
                        <li><button data-category="all" class="hover:text-primary" aria-label="عرض الكل">الكل</button></li>
                        @foreach ($categories as $category)
                            <li>
                                <button data-category="{{ trim(Str::lower($category->slug)) }}" class="hover:text-primary" aria-label="عرض قوالب {{ $category->translated_name }}">
                ...
                <div class="border border-gray-200 dark:border-gray-700 p-4 rounded-xl shadow-md">
                    ...
                    <div class="mb-4">
                        <input id="priceRange" type="range" min="0" max="{{ (int) $max_price }}" value="{{ (int) $max_price }}" class="w-full accent-primary">
                    </div>
                    <p class="text-sm text-gray-700">السعر الأقصى: <span id="priceValue">{{ (int) $max_price }}</span>$</p>
                </div>
            </aside>
            <!-- شبكة القوالب -->
            <div class="lg:col-span-3">
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <select id="sortSelect" class="border border-gray-300 text-sm rounded px-3 py-1">
                        <option value="default" {{ ($sort_by ?? 'default') === 'default' ? 'selected' : '' }}>الترتيب الافتراضي</option>
                        <option value="high" {{ ($sort_by ?? '') === 'high'    ? 'selected' : '' }}>الأعلى سعرًا</option>
                        <option value="low" {{ ($sort_by ?? '') === 'low'     ? 'selected' : '' }}>الأقل سعرًا</option>
                    </select>
                </div>
                <!-- الشبكة -->
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6" id="templateGrid">
                    @forelse ($templates as $template)
                        ...
                        @endphp
                        <a
                            href="{{ route('template.show', $translation->slug) }}"
                            data-card data-category="{{ trim(Str::lower($template->categoryTemplate?->slug ?? 'uncategorized')) }}"
                            class="block group"
                            data-price="{{ $template->discount_price ?? $template->price }}"
                        >
                            <article style="will-change: transform" class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden relative group transition-transform duration-300 hover:-translate-y-2 hover:shadow-2xl animate-fade-in-up border border-primary/10" itemscope itemtype="https://schema.org/Product" lang="ar">
                                ...
                                <div class="relative">
                                    <img itemprop="image" src="{{ asset('storage/' . $template->image) }}" alt="{{ $translation->name }}" class="w-full h-40 object-cover transition-transform duration-300 group-hover:scale-105 group-hover:brightness-95" loading="lazy" decoding="async">
                                ...
                                <div class="p-5 rtl:text-right ltr:text-left flex flex-col items-start">
                                    <h3 itemprop="name" class="text-suptitle font-bold mb-1 text-primary/90 dark:text-white group-hover:text-secondary transition-colors leading-snug">{{ $translation->name }}</h3>
                                    <p itemprop="description" class="text-suptitle font-light mb-2 text-primary/70 dark:text-gray-300">{{ Str::limit(strip_tags($translation->description), 70) }}</p>
                                    <div class="flex justify-between items-center text-sm font-bold rtl:flex-row-reverse ltr:flex-row mt-3 w-full">
                                        <div class="flex items-center gap-1" aria-label="التقييم 4 من 5 نجوم">
                                            <span class="text-yellow-400 text-base">{!! str_repeat('★', floor($template->rating)) !!}{!! str_repeat('☆', 5 - floor($template->rating)) !!}</span>
                                        </div>
