<div class="bg-white dark:bg-gray-800 rounded p-6 shadow mb-8 border">
    ...
    <div wire:key="works-{{ $activeLang }}" class="space-y-4">
        ...
        {{-- المميزات --}}
        <div class="space-y-4">
            @foreach ($translationsData[$activeLang]['works'] ?? [] as $index => $feature)
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start border p-4 rounded relative">
                    <input type="text" wire:model="translationsData.{{ $activeLang }}.works.{{ $index }}.icon"
                        placeholder="كود SVG أو اسم الأيقونة"
                        class="form-input w-full px-4 py-2 rounded border" />

                    <input type="text" wire:model="translationsData.{{ $activeLang }}.works.{{ $index }}.title"
                        placeholder="عنوان الميزة"
                        class="form-input w-full px-4 py-2 rounded border" />

                    <input type="text" wire:model="translationsData.{{ $activeLang }}.works.{{ $index }}.description"
                        placeholder="وصف مختصر"
                        class="form-input w-full px-4 py-2 rounded border" />

                    <button wire:click="removeworks('{{ $activeLang }}', {{ $index }})"
