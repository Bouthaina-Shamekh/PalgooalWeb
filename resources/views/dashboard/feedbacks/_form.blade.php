...
<div class="col-span-6">
    ...
    <div class="flex items-center gap-2">
        <input type="hidden" id="iconInput" name="image" value="{{ old('image', $feedback->image ?? '') }}" class="form-control hidden">
...
<div class="col-span-6">
    ...
    <input type="number" name="order" value="{{ old('order', $feedback->order ?? '') }}" class="form-control">
    @error('order')
...
<div class="col-span-6">
    ...
    <input type="number" name="star" value="{{ old('star', $feedback->star ?? '') }}" class="form-control">
    @error('star')
        <span class="text-red-600">{{ $message }}</span>
    @enderror
</div>
...
<div class="col-span-12 grid grid-cols-{{ count($languages) }} gap-4">
    ...
    @php
    $translation = $feedbackTranslations[$lang->code] ?? null;
@endphp
        <div class="border p-4 rounded shadow-sm">
            <h4 class="text-lg font-bold mb-2">{{ $lang->native }}</h4>

            <input type="text" class="form-control mb-2" placeholder="الاسم" name="feedbackTranslations[{{ $lang->code }}][name]" value="{{ old('feedbackTranslations[' . $lang->code . '][name]', $translation['name'] ?? '') }}">

            <textarea class="form-control" rows="3" placeholder="النص" name="feedbackTranslations[{{ $lang->code }}][feedback]">{{ old('feedbackTranslations[' . $lang->code . '][feedback]', $translation['feedback'] ?? '') }}</textarea>

            <input type="text" class="form-control mb-2" placeholder="المهنة" name="feedbackTranslations[{{ $lang->code }}][major]" value="{{ old('feedbackTranslations[' . $lang->code . '][major]', $translation['major'] ?? '') }}">

            <input type="hidden" name="feedbackTranslations[{{ $lang->code }}][locale]" value="{{ $lang->code }}">
        </div>
    @endforeach
</div>

<div class="col-span-12 text-right mt-6">
    <a href="{{ route('dashboard.feedbacks.index') }}" class="btn btn-secondary">إلغاء</a>
