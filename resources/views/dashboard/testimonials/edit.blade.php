    ...
    <div class="page-header">
        <div class="page-block">
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{ t('dashboard.Edit_Testimonial', 'Edit Testimonial') }}</li>
            </ul>
            <div class="page-header-title">
                <h2 class="mb-0">
                    {{ t('dashboard.Edit_Testimonial', 'Edit Testimonial') }}
                    @php
                        $trans = $testimonial->translations->firstWhere('locale', app()->getLocale())
                              ?? $testimonial->translations->first();
                    @endphp
                    @if ($trans?->name)
                        <span class="text-gray-400 font-normal text-lg">— {{ $trans->name }}</span>
                    @endif
                </h2>
            </div>
        </div>
