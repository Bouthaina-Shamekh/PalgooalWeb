        ...
        <div class="col-span-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form action="{{ route('dashboard.subscriptions.update', $subscription) }}" method="POST" class="grid grid-cols-12 gap-x-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <input type="text" name="username" class="form-control" value="{{ old('username', $subscription->username) }}">
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <select name="server_id" class="form-select" required>
                                ...
                                @foreach($servers as $server)
                                    <option value="{{ $server->id }}" {{ old('server_id', $subscription->server_id) == $server->id ? 'selected' : '' }}>{{ $server->name }} ({{ $server->ip ?? $server->hostname }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <input type="date" name="next_due_date" class="form-control" value="{{ old('next_due_date', $subscription->next_due_date) }}">
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <select name="client_id" class="form-select" required>
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" {{ old('client_id', $subscription->client_id) == $client->id ? 'selected' : '' }}>{{ $client->first_name }} {{ $client->last_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <select name="domain_option" class="form-select" required>
                                <option value="new" {{ old('domain_option', $subscription->domain_option) == 'new' ? 'selected' : '' }}>تسجيل دومين جديد</option>
                                <option value="subdomain" {{ old('domain_option', $subscription->domain_option) == 'subdomain' ? 'selected' : '' }}>استخدام سب-دومين</option>
                                <option value="existing" {{ old('domain_option', $subscription->domain_option) == 'existing' ? 'selected' : '' }}>دومين خاص بالعميل</option>
                            </select>
                            @error('domain_option') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-span-12 md:col-span-6">
                            <label class="form-label">اسم الدومين</label>
                            <input type="text" name="domain_name" class="form-control" value="{{ old('domain_name', $subscription->domain_name) }}" placeholder="مثال: example.com أو client.palgoals.com">
