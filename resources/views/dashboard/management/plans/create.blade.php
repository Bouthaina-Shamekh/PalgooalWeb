                ...
                <div class="card-body">
                    ...
                    <form id="planForm" action="{{ route('dashboard.plans.store') }}" method="POST"
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <div class="flex">
                                <span
                                    class="inline-flex items-center px-3 rounded-s-xl border border-e-0 bg-gray-50">$</span>
                                <input type="number" step="0.01" min="0" id="monthly_price_ui"
                                    name="monthly_price_ui" class="form-control rounded-s-none"
                                    value="{{ old('monthly_price_ui') ?? '' }}" placeholder="0.00">
                            </div>
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <div class="flex">
                                ...
                                <input type="number" step="0.01" min="0" id="annual_price_ui"
                                    name="annual_price_ui" class="form-control rounded-s-none"
                                    value="{{ old('annual_price_ui') ?? '' }}" placeholder="0.00">
                            </div>
                        ...
                        <div class="col-span-12">
                            ...
                            <div class="flex gap-2">
                                <input id="featureInput" type="text" class="form-control"
                                    placeholder="e.g. 10GB SSD, Free SSL, Email accounts">
                            ...
                            <div id="featuresChips" class="mt-3 flex flex-wrap gap-2">
                                ...
                                @foreach ($oldFeatures as $f)
                                    <span
                                        class="badge bg-success-500/10 text-success-700 rounded-full px-3 py-1 flex items-center gap-2">
                                        <span>{{ $f }}</span>
                                        <button type="button" class="text-red-600 remove-chip"
                                            data-value="{{ $f }}">✕</button>
                            ...
                            <select name="server_id" class="form-select">
                                ...
                                @foreach ($servers as $server)
                                    <option value="{{ $server->id }}"
                                        {{ old('server_id') == $server->id ? 'selected' : '' }}>{{ $server->name }}
                                        ({{ $server->ip ?? $server->hostname }})</option>
                                @endforeach
                            </select>
                            @error('server_id')
                                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
