<div class="lg:col-span-2">
    <div data-pricing-plan-repeater data-plan-item-label="{{ __('Plan') }}"
        ...
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">
                    {{ __('Each plan card belongs to a category tab and contains its own feature list and CTA button.') }}
                </p>
            </div>
            <button type="button" data-add-pricing-plan
        ...
        <div class="mt-3">
            <div class="space-y-3" data-pricing-plan-items>
                @foreach ($pricingPlanItems as $planIndex => $planItem)
                    @php
                        ...
                        $planSummaryParts = array_values(
                            array_filter([
                                filled($planItem['category'] ?? '') ? __('Tab') . ': ' . $planItem['category'] : null,
                                $planFeaturesCount
                                    ? $planFeaturesCount .
                                        ' ' .
                                        \Illuminate\Support\Str::plural('feature', $planFeaturesCount)
                                    : null,
                                filled($planItem['button_label'] ?? '')
                                    ? __('CTA') . ': ' . $planItem['button_label']
                                    : null,
                                !empty($planItem['button_new_tab']) ? __('Opens in a new tab') : null,
                            ]),
                        );
                    @endphp
                    <article data-pricing-plan-item
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between gap-3 rtl:flex-row-reverse">
                                <button type="button" data-pricing-plan-drag-handle
                                ...
                                <div class="flex shrink-0 items-center gap-2 rtl:flex-row-reverse">
                                    <button type="button" data-duplicate-pricing-plan
                                    ...
                                    <button type="button" data-remove-pricing-plan
                            ...
                            <button type="button" data-pricing-plan-toggle
                                ...
                                <span
                                    ...
                                    <i data-pricing-plan-toggle-icon
                        ...
                        <div data-pricing-plan-body class="mt-4 space-y-4 {{ $loop->first ? '' : 'hidden' }}">
                            <div>
                                <div class="flex items-center justify-between gap-3 rtl:flex-row-reverse">
                                    <label
                                        class="block text-sm font-medium text-slate-700">{{ __('Plan Name') }}</label>
                                ...
                                <input type="text"
                                    ...
                                    data-pricing-plan-field="title" value="{{ $planItem['title'] ?? '' }}"
                            ...
                            <div>
                                <div class="flex items-center justify-between gap-3 rtl:flex-row-reverse">
                                    ...
                                    <span
                                        class="text-xs text-slate-400">{{ __('Choose one of the tabs above') }}</span>
                                </div>
                                <input type="text" list="{{ $pricingCategoryDatalistId }}"
                                    ...
                                    data-pricing-plan-field="category" value="{{ $planItem['category'] ?? '' }}"
                                    class="mt-2 block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                                    placeholder="shared">
                            </div>
                            ...
                            <div>
                                <div class="flex items-center justify-between gap-3 rtl:flex-row-reverse">
                                    <label
                                        class="block text-sm font-medium text-slate-700">{{ __('Plan Features') }}</label>
                                    <span
                                        class="text-xs text-slate-400">{{ __('Each line becomes one list item') }}</span>
                                </div>
                                <textarea name="translations[{{ $code }}][content][plans][{{ $planIndex }}][features_textarea]"
                                    data-name-template="translations[{{ $code }}][content][plans][__INDEX__][features_textarea]"
                                    data-pricing-plan-field="features" rows="6"
                                    class="mt-2 block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                                    placeholder="{{ __('20 GB SSD Cloud storage') }}&#10;{{ __('5 hosted domains') }}">{{ $planItem['features_textarea'] ?? '' }}</textarea>
                            </div>
                            ...
                            <div>
                                <label
                                    ...
                                    <input type="checkbox"
                                        ...
                                        data-pricing-plan-field="button_new_tab" value="1"
            ...
            <div data-pricing-plan-empty
                class="{{ count($pricingPlanItems) ? 'hidden ' : '' }}mt-3 rounded-2xl border border-dashed border-slate-300 bg-white/80 px-4 py-6 text-center text-sm text-slate-500">
        ...
        <template data-pricing-plan-template>
            <article data-pricing-plan-item
                ...
                <div class="space-y-3">
                    ...
                    <button type="button" data-pricing-plan-toggle aria-expanded="false"
                        class="flex w-full min-w-0 items-start justify-between gap-3 rounded-2xl bg-slate-50/80 px-3 py-3 text-left transition hover:bg-slate-100 rtl:flex-row-reverse rtl:text-right">
                        <div class="min-w-0 flex-1">
                            <p dir="auto" data-pricing-plan-title
                                class="text-sm font-semibold leading-5 text-slate-900 break-words">
                            ...
                            <p dir="auto" data-pricing-plan-summary
                                class="mt-1 text-xs leading-5 text-slate-500 break-words">
                                {{ __('Click to edit this plan card') }}</p>
                        </div>

                        <span
                            class="mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500">
                            <i data-pricing-plan-toggle-icon class="ti ti-chevron-down text-base leading-none"
                                aria-hidden="true"></i>
                        </span>
                    </button>
                </div>
                ...
                <div data-pricing-plan-body class="mt-4 hidden space-y-4">
                    <div>
                        ...
                        <input type="text"
                            ...
                            data-name-template="translations[{{ $code }}][content][plans][__INDEX__][title]"
                            data-pricing-plan-field="title" value=""
                            class="mt-2 block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                            placeholder="{{ __('PLAN1') }}">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" list="{{ $pricingCategoryDatalistId }}"
                            ...
                            data-name-template="translations[{{ $code }}][content][plans][__INDEX__][category]"
                            data-pricing-plan-field="category" value=""
                            class="mt-2 block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                            placeholder="shared">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="translations[{{ $code }}][content][plans][__INDEX__][features_textarea]"
                            data-name-template="translations[{{ $code }}][content][plans][__INDEX__][features_textarea]"
                            data-pricing-plan-field="features" rows="6"
                            class="mt-2 block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                            placeholder="{{ __('20 GB SSD Cloud storage') }}&#10;{{ __('5 hosted domains') }}"></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                        <div>
                            ...
                            <input type="text"
                                ...
                                data-name-template="translations[{{ $code }}][content][plans][__INDEX__][button_label]"
                                data-pricing-plan-field="button_label" value=""
                                class="mt-2 block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                                placeholder="{{ __('Choose Now') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text"
                                ...
                                data-name-template="translations[{{ $code }}][content][plans][__INDEX__][button_url]"
                                data-pricing-plan-field="button_url" value=""
                                class="mt-2 block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                                placeholder="#">
                        </div>
                    </div>
