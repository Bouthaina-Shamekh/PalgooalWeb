<x-dashboard-layout>
    <div class="container mx-auto py-6 max-w-5xl">
        ...
        <form action="{{ route('dashboard.templates.update', $template->id) }}" method="POST"
            enctype="multipart/form-data" class="space-y-6 bg-white p-6 shadow rounded-lg">
            @csrf
            @method('PUT')
                ...
                <select name="category_template_id" required class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}"
                            {{ $template->category_template_id == $category->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="plan_id" required class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($plans as $plan)
                        <option value="{{ $plan->id }}" {{ $template->plan_id == $plan->id ? 'selected' : '' }}>
                            {{ $plan->name }} ({{ number_format($plan->price_cents / 100, 2) }} $)</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="price" step="0.01" value="{{ old('price', $template->price) }}"
                        ...
                        <div class="border rounded p-4" data-locale-section>
                            ...
                            <div class="mb-2">
                                ...
                                <input type="url" name="translations[{{ $loop->index }}][preview_url]"
                                    value="{{ old("translations.$loop->index.preview_url", $translation?->preview_url) }}"
                                    class="w-full border p-2 rounded" />
                            </div>
                            <div class="mb-2">
                                ...
                                <textarea name="translations[{{ $loop->index }}][description]" rows="4" class="w-full border p-2 rounded"
                                    required>{{ old("translations.$loop->index.description", $translation?->description) }}</textarea>
                            </div>
