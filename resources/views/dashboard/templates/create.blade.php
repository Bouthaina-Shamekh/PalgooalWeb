<x-dashboard-layout>
    ...
    <div class="min-h-screen bg-slate-50">
        <div class="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
            <section
                class="relative overflow-hidden rounded-[32px] bg-slate-950 px-6 py-8 text-white shadow-2xl shadow-slate-900/10 sm:px-8 lg:px-10">
                <div
                    class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(168,85,247,0.35),_transparent_35%),radial-gradient(circle_at_bottom_left,_rgba(59,130,246,0.22),_transparent_28%)]">
                </div>
                <div class="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                    <div class="max-w-3xl text-right">
                        <span
                            class="inline-flex items-center rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold tracking-[0.24em] text-violet-100">
                        ...
                        <div class="mt-0.5 rounded-full bg-rose-100 p-2 text-rose-600">
                            <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                <path fill-rule="evenodd"
                                    d="M18 10A8 8 0 1 1 2 10a8 8 0 0 1 16 0Zm-8.75-3a.75.75 0 0 0 1.5 0 .75.75 0 0 0-1.5 0ZM10 8.75a.75.75 0 0 0-.75.75v4a.75.75 0 0 0 1.5 0v-4A.75.75 0 0 0 10 8.75Z"
                                    clip-rule="evenodd" />
                            </svg>
                        </div>
                        <div class="min-w-0">
                            <p class="font-bold">
                                {{ t('dashboard.templates.form.save_failed', 'تعذر حفظ القالب. راجع الحقول التالية ثم أعد المحاولة.') }}
                            </p>
            ...
            <form action="{{ route('dashboard.templates.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="space-y-8">
                    <section
                        class="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm shadow-slate-200/60">
                        <div class="border-b border-slate-200 px-6 py-5 sm:px-8">
                            <span
                                class="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-xs font-bold tracking-[0.18em] text-slate-600">
                                {{ t('dashboard.templates.form.core_setup_badge', 'CORE SETUP') }}
                            </span>
                            <h2 class="mt-3 text-2xl font-black tracking-tight text-slate-900">
                                {{ t('dashboard.templates.form.basic_settings_title', 'الإعدادات الأساسية') }}</h2>
                        ...
                        <div class="grid gap-6 px-6 py-6 sm:px-8 lg:grid-cols-2">
                            <label class="block">
                                <span
                                    class="mb-2 block text-sm font-semibold text-slate-700">{{ t('dashboard.templates.form.category_label', 'تصنيف القالب') }}</span>
                                <select name="category_template_id" required
                                    ...
                                    <option value="">
                                        {{ t('dashboard.templates.form.choose_category', 'اختر التصنيف') }}</option>
                            ...
                            <label class="block">
                                <span
                                    class="mb-2 block text-sm font-semibold text-slate-700">{{ t('dashboard.templates.form.plan_label', 'الخطة المرتبطة') }}</span>
                                <select name="plan_id" required
                                    class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-900 outline-none transition focus:border-violet-400 focus:bg-white focus:ring-4 focus:ring-violet-100">
                                    <option value="">
                                        {{ t('dashboard.templates.form.choose_plan', 'اختر الخطة') }}</option>
                                        ...
                                        <option value="{{ $plan->id }}" @selected(old('plan_id') == $plan->id)>
                                            ...
                                            @if ($plan->monthly_price_formatted || $plan->annual_price_formatted)
                                                ...
                                                @if ($plan->monthly_price_formatted)
                                                    {{ t('dashboard.templates.form.monthly', 'شهري') }}
                                                    {{ $plan->monthly_price_formatted }}
                                                @endif
                                                @if ($plan->monthly_price_formatted && $plan->annual_price_formatted)
                                                    /
                                                @endif
                                                @if ($plan->annual_price_formatted)
                                                    {{ t('dashboard.templates.form.yearly', 'سنوي') }}
                                                    {{ $plan->annual_price_formatted }}
                                                @endif
                                                )
                                            @endif
                                        </option>
                            ...
                            <label class="block">
                                <span
                                    class="mb-2 block text-sm font-semibold text-slate-700">{{ t('dashboard.templates.form.base_price', 'السعر الأساسي ($)') }}</span>
                            ...
                            <label class="block">
                                <span
                                    class="mb-2 block text-sm font-semibold text-slate-700">{{ t('dashboard.templates.form.discount_price', 'سعر الخصم ($)') }}</span>
                            ...
                            <label class="block lg:col-span-2">
                                <span
                                    class="mb-2 block text-sm font-semibold text-slate-700">{{ t('dashboard.templates.form.discount_ends_at', 'تاريخ انتهاء الخصم') }}</span>
                            ...
                            <div
                                class="lg:col-span-2 rounded-[24px] border border-dashed border-slate-300 bg-slate-50 px-5 py-5">
                                <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-center">
                                    <div>
                                        <p class="text-sm font-semibold text-slate-900">
                                            {{ t('dashboard.templates.form.main_image_title', 'صورة القالب الرئيسية') }}
                                        </p>
                                        <p class="mt-2 text-sm leading-7 text-slate-500">
                                    ...
                                    <div class="block">
                                        <span
                                            class="sr-only">{{ t('dashboard.templates.form.main_image_sr', 'صورة القالب') }}</span>
                                        <x-dashboard.media-picker id="template_image_media_id" name="image_media_id"
                                            :label="t(
                                                'dashboard.templates.form.main_image_picker_label',
                                                'اختر صورة القالب من مكتبة الميديا',
                                            )" :value="$selectedImageMediaId" :preview-urls="$selectedImagePreviewUrls" :button-text="t(
                                                'dashboard.templates.form.choose_from_media_picker',
                                                'اختيار من Media Picker',
                                            )"
                                            class="col-span-12" />
                    ...
                    <section
                        ...
                        <div class="border-b border-slate-200 px-6 py-5 sm:px-8">
                            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                                <div>
                                    <span
                                        class="inline-flex items-center rounded-full bg-violet-100 px-3 py-1 text-xs font-bold tracking-[0.18em] text-violet-700">
                                        {{ t('dashboard.templates.form.translations_badge', 'TRANSLATIONS') }}
                                    </span>
                                    <h2 class="mt-3 text-2xl font-black tracking-tight text-slate-900">
                                        {{ t('dashboard.templates.form.translation_content_title', 'بيانات الترجمة والمحتوى') }}
                                    </h2>
                                    <p class="mt-2 text-sm leading-7 text-slate-500">
                                ...
                                <div class="w-full lg:w-auto lg:min-w-[440px]">
                                    ...
                                    <div class="grid gap-2 sm:grid-cols-2">
                                        @foreach ($languages as $language)
                                            ...
                                            @endphp
                                            <button type="button" data-locale-tab="{{ $locale }}"
                                                ...
                                                <span class="flex flex-1 items-center gap-3">
                                                    ...
                                                    <span class="min-w-0 flex-1 text-right">
                                                        <span
                                                            class="block text-sm font-black">{{ $language->name }}</span>
                                                ...
                                                <span data-locale-check="{{ $locale }}"
                                                    ...
                                                    <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"
                                                        aria-hidden="true">
                                                        <path fill-rule="evenodd"
                                                            d="M16.704 5.29a1 1 0 0 1 .006 1.414l-7.2 7.261a1 1 0 0 1-1.42-.008L4.3 10.08a1 1 0 1 1 1.4-1.428l2.998 2.94 6.5-6.55a1 1 0 0 1 1.414-.006Z"
                                                            clip-rule="evenodd" />
                                                    </svg>
                                                </span>
                                            </button>
                        ...
                        <div class="space-y-6 px-6 py-6 sm:px-8">
                            @foreach ($languages as $language)
                                @php
                                    ...
                                    $initialDetailsDecoded =
                                        is_string($initialDetailsRaw) && $initialDetailsRaw !== ''
                                            ? json_decode($initialDetailsRaw, true)
                                            : [];
                                    $initialGalleryPaths = collect(
                                        is_array($initialDetailsDecoded) ? $initialDetailsDecoded['gallery'] ?? [] : [],
                                    )
                                    ...
                                    $initialDashboardPayload = is_array($initialDetailsDecoded['specs'] ?? null)
                                        ...
                                        : [];
                                    $initialDashboardDescription = trim(
                                        (string) ($initialDashboardPayload['description'] ?? ''),
                                    );
                                    $initialDashboardPaths = collect(
                                        is_array($initialDashboardPayload['images'] ?? null)
                                            ? $initialDashboardPayload['images'] ?? []
                                            : [],
                                    )
                                        ->map(fn($item) => is_array($item) ? trim((string) ($item['src'] ?? '')) : '')
                                        ->filter()
                                ...
                                <div data-locale-panel="{{ $locale }}"
                                    ...
                                    <div
                                        ...
                                        <div>
                                            ...
                                            <p class="mt-1 text-sm text-slate-500">
                                                {{ t('dashboard.templates.form.locale_section_intro_prefix', 'جميع الحقول أدناه تخص لغة') }}
                                                {{ $language->name }}
                                                {{ t('dashboard.templates.form.locale_section_intro_suffix', 'فقط.') }}
                                            </p>
                                        </div>
                                        <span
                                            class="inline-flex items-center self-start rounded-full bg-white px-3 py-1 text-xs font-bold tracking-[0.18em] text-slate-600">
                                            {{ strtoupper($locale) }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="grid gap-5 lg:grid-cols-2">
                                        <label class="block">
                                            <span
                                                class="mb-2 block text-sm font-semibold text-slate-700">{{ t('dashboard.templates.form.template_name', 'اسم القالب') }}</span>
                                            <input type="text" name="translations[{{ $i }}][name]"
                                                ...
                                                placeholder="{{ t('dashboard.templates.form.template_name_placeholder', 'Single Product Ecommerce') }}" />
                                        </label>

                                        <div class="block">
                                            <label
                                                class="mb-2 flex items-center justify-between gap-3 text-sm font-semibold text-slate-700">
                                                <span>{{ t('dashboard.templates.form.slug_label', 'الرابط المختصر (slug)') }}</span>
                                                <button type="button"
                                                    class="generate-slug text-xs font-bold text-violet-700 transition hover:text-violet-900">
                                            ...
                                            <input type="text" name="translations[{{ $i }}][slug]"
                                                ...
                                                placeholder="single-product-ecommerce" />
                                            <p class="mt-2 text-xs text-slate-500">
                                                {{ t('dashboard.templates.form.slug_help', 'يستخدم هذا الرابط في المسار العام لصفحة القالب.') }}
                                            </p>
                                        </div>

                                        <label class="block lg:col-span-2">
                                            <span
                                                class="mb-2 block text-sm font-semibold text-slate-700">{{ t('dashboard.templates.form.preview_url', 'رابط المعاينة') }}</span>
                                            <input type="url"
                                        ...
                                        <label class="block lg:col-span-2">
                                            <span
                                                class="mb-2 block text-sm font-semibold text-slate-700">{{ t('dashboard.templates.form.description', 'الوصف') }}</span>
                                            <textarea name="translations[{{ $i }}][description]" rows="5"
                                                class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-900 outline-none transition focus:border-violet-400 focus:ring-4 focus:ring-violet-100"
                                                required
                                                placeholder="{{ t('dashboard.templates.form.description_placeholder', 'اكتب وصفًا مختصرًا يشرح وظيفة القالب وأبرز ما يقدمه.') }}">{{ old("translations.$i.description") }}</textarea>
                                        </label>
                                    </div>
                                    ...
                                    <div class="mt-6 grid gap-6 2xl:grid-cols-2">
                                            ...
                                            <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                                                <div>
                                                    <h4 class="text-base font-black text-slate-900">
                                                        {{ t('dashboard.templates.form.features_title', 'المميزات') }}
                                                    </h4>
                                                    <p class="mt-1 text-xs text-slate-500">
                                                        {{ t('dashboard.templates.form.features_intro', 'أضف نقاط البيع الأساسية التي ستظهر للمستخدم.') }}
                                                    </p>
                                                </div>
                                        ...
                                        <div class="rounded-[22px] border border-slate-200 bg-white p-4 sm:p-5"
                                            ...
                                            <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                                                <div>
                                                    <h4 class="text-base font-black text-slate-900">
                                                        {{ t('dashboard.templates.form.gallery_title', 'المعرض') }}
                                                    </h4>
                                                    <p class="mt-1 text-xs text-slate-500">
                                                        {{ t('dashboard.templates.form.gallery_intro', 'روابط صور الشاشات أو الأمثلة الخاصة بالقالب.') }}
                                                    </p>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <button type="button"
                                            ...
                                            <div
                                                class="mb-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                                                <x-dashboard.media-picker id="gallery_picker_{{ $i }}"
                                                    name="gallery_picker_helper_{{ $i }}" :label="t(
                                                        'dashboard.templates.form.gallery_picker_label',
                                                        'اختيار صور من مكتبة الميديا',
                                                    )"
                                                    :value="$initialGalleryPaths" :preview-urls="$initialGalleryPreviewUrls" :multiple="true"
                                                    store-value="path" :button-text="t(
                                                        'dashboard.templates.form.choose_multiple_from_media_picker',
                                                        'اختيار متعدد من Media Picker',
                                                    )" class="col-span-12" />
                                                <p class="mt-3 text-xs leading-6 text-slate-500">
                                        ...
                                        <div class="rounded-[22px] border border-slate-200 bg-white p-4 sm:p-5"
                                            ...
                                            <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                                                <div>
                                                    <h4 class="text-base font-black text-slate-900">
                                                        {{ t('dashboard.templates.form.development_tools_title', 'الأدوات المستخدمة في التطوير') }}
                                                    </h4>
                                                    <p class="mt-1 text-xs text-slate-500">
                                                        {{ t('dashboard.templates.form.development_tools_intro', 'أضف اسم الأداة أو التقنية مع شعارها من مكتبة الميديا ليتم عرضها في قسم Used in development.') }}
                                                    </p>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <button type="button"
                                        ...
                                        <div class="rounded-[22px] border border-slate-200 bg-white p-4 sm:p-5"
                                            ...
                                            <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                                                <div>
                                                    <h4 class="text-base font-black text-slate-900">
                                                        {{ t('dashboard.templates.form.dashboard_title', 'Dashboard') }}
                                                    </h4>
                                                    <p class="mt-1 text-xs text-slate-500">
                                                        {{ t('dashboard.templates.form.dashboard_intro', 'أضف وصفًا وصورًا متعددة لعرضها داخل قسم Dashboard في صفحة القالب.') }}
                                                    </p>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <button type="button"
                                            ...
                                            <div
                                                class="mb-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                                                <x-dashboard.media-picker id="dashboard_picker_{{ $i }}"
                                                    name="dashboard_picker_helper_{{ $i }}"
                                                    :label="t(
                                                        'dashboard.templates.form.dashboard_picker_label',
                                                        'اختيار صور الـ Dashboard من مكتبة الميديا',
                                                    )" :value="$initialDashboardPaths" :preview-urls="$initialDashboardPreviewUrls"
                                                    :multiple="true" store-value="path" :button-text="t(
                                                        'dashboard.templates.form.choose_multiple_from_media_picker',
                                                        'اختيار متعدد من Media Picker',
                                                    )"
                                                    class="col-span-12" />
                                    ...
                                    <div class="mt-6 rounded-[22px] border border-slate-200 bg-white p-4 sm:p-5"
                                        data-browsers-wrapper>
                                        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                                            <div>
                                                <h4 class="text-base font-black text-slate-900">
                                                    {{ t('dashboard.templates.form.compatible_browsers_title', 'Compatible Browsers') }}
                                                </h4>
                                                <p class="mt-1 text-xs text-slate-500">
                                                    {{ t('dashboard.templates.form.compatible_browsers_intro', 'اختر المتصفحات المتوافقة مع القالب، ويمكنك إضافة متصفح مخصص إذا لزم.') }}
                                                </p>
                                            </div>
                                            <button type="button"
                                    ...
                                    <div class="mt-6 rounded-[22px] border border-slate-200 bg-white p-4 sm:p-5"
                                        ...
                                        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                                            <div>
                                                <h4 class="text-base font-black text-slate-900">
                                                    {{ t('dashboard.templates.form.tags_title', 'الوسوم') }}</h4>
                                                <p class="mt-1 text-xs text-slate-500">
                                                    {{ t('dashboard.templates.form.tags_intro', 'أضف كلمات مفتاحية قصيرة تساعد في تنظيم القالب وعرضه.') }}
                                                </p>
                                            </div>
                                        </div>
                                        <div class="mb-3 flex flex-col gap-3 sm:flex-row">
                                    ...
                                    <input type="hidden" name="translations[{{ $i }}][details]"
                                        class="details-json" value="{{ old("translations.$i.details", '') }}">
                ...
                <aside class="space-y-6 xl:sticky xl:top-8 xl:self-start">
                    <section
                        ...
                        <div class="border-b border-slate-200 px-6 py-5">
                            <span
                                class="inline-flex items-center rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-xs font-semibold tracking-[0.18em] text-violet-700">
                                {{ t('dashboard.templates.form.ready_to_save_badge', 'READY TO SAVE') }}
                            </span>
                            <h2 class="mt-3 text-2xl font-black tracking-tight text-slate-900">
                                {{ t('dashboard.templates.form.save_actions_title', 'إجراءات الحفظ') }}</h2>
                        ...
                        <div class="space-y-5 px-6 py-6">
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                <p class="text-xs font-semibold tracking-[0.18em] text-slate-500">
                                    {{ t('dashboard.templates.form.quick_summary', 'ملخص سريع') }}</p>
                                <div class="mt-4 space-y-3 text-sm text-slate-700">
                                    <div class="flex items-center justify-between gap-3">
                                        <span>{{ t('dashboard.templates.form.translation_languages', 'لغات الترجمة') }}</span>
                                        <span
                                            class="rounded-full bg-white px-2.5 py-1 text-xs font-bold text-slate-900 shadow-sm ring-1 ring-slate-200">{{ number_format($languages->count()) }}</span>
                                    </div>
                                    <div class="flex items-center justify-between gap-3">
                                        <span>{{ t('dashboard.templates.form.available_categories', 'التصنيفات المتاحة') }}</span>
                                        <span
                                            class="rounded-full bg-white px-2.5 py-1 text-xs font-bold text-slate-900 shadow-sm ring-1 ring-slate-200">{{ number_format($categories->count()) }}</span>
                                    </div>
                                    <div class="flex items-center justify-between gap-3">
                                        <span>{{ t('dashboard.templates.form.available_plans', 'الخطط المتاحة') }}</span>
                                        <span
                                            class="rounded-full bg-white px-2.5 py-1 text-xs font-bold text-slate-900 shadow-sm ring-1 ring-slate-200">{{ number_format($plans->count()) }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid gap-4 pt-3">
                                <button type="submit"
                                    ...
                                    <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                        <path
                                            d="M10 2a1 1 0 0 1 1 1v1.293l4.854 4.853a.5.5 0 0 1 .146.354V16a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V9.5a.5.5 0 0 1 .146-.354L9 4.293V3a1 1 0 0 1 1-1Z" />
                                    </svg>
                    ...
                    <section class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60">
                        <h3 class="text-lg font-black text-slate-900">
                            {{ t('dashboard.templates.form.before_save', 'قبل الحفظ') }}</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const activeClasses = ['border-violet-300', 'bg-violet-50', 'text-violet-950', 'shadow-sm', 'ring-2',
                'ring-violet-100'
            ];
            ...
            activateLocale(tabs.find((tab) => tab.getAttribute('aria-pressed') === 'true')?.dataset.localeTab ||
                tabs[0].dataset.localeTab);
        });
    </script>
    ...
    @php
        $templateCreateI18n = [
            ...
            'chooseImageFromLibrary' => t(
                'dashboard.templates.form.js.choose_image_from_library',
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const i18n = @json($templateCreateI18n, JSON_UNESCAPED_UNICODE);
            ...
            document.querySelectorAll('[data-locale-section]').forEach(section => {
                ...
                function renderGalleryPickerPreview(paths = []) {
                    ...
                    paths.forEach((path) => {
                        ...
                        wrapper.className =
                            'relative h-20 w-20 overflow-hidden rounded-lg border border-gray-200 bg-gray-50';
                        wrapper.innerHTML =
                            `<img src="${escapeHtml(previewUrl)}" alt="" class="h-full w-full object-cover">`;
                ...
                function featureRow(item = {
                    ...
                    row.className =
                        'feature-row grid grid-cols-1 gap-2 rounded-2xl border border-slate-200 p-3 sm:grid-cols-[1fr_160px_auto]';
                    row.innerHTML = `
                        <input type="text"
                ...
                function imageRow(item = {
                    ...
                    const row = document.createElement('div');
                    row.className =
                        'image-row grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 p-3 sm:grid-cols-[88px_1fr_auto] sm:items-center';
                    row.innerHTML = `
                        <input type="hidden"
                               class="img-src"
                ...
                function specRow(item = {
                    ...
                    const row = document.createElement('div');
                    row.className =
                        'spec-row grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 p-3 sm:grid-cols-[88px_1fr_auto] sm:items-center';
                    row.innerHTML = `
                        <input type="hidden"
                               class="spec-img-src"
                               value="${escapeHtml(item.src)}">
                ...
                function detailRow(item = {
                    ...
                    const row = document.createElement('div');
                    row.className =
                        'detail-row grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 p-3 sm:grid-cols-[88px_minmax(0,1fr)_auto] sm:items-center';
                    row.innerHTML = `
                        <input type="hidden"
                               class="detail-src"
                               id="${mediaInputId}"
                    ...
                    const syncDetailImage = () => {
                        ...
                        if (detailFileLabel) {
                            detailFileLabel.textContent = src ? galleryFileName(src) : i18n
                                .noImageSelected;
                        }
                        if (detailPreview) {
                ...
                function deriveLegacyBrowsers(existing = {}) {
                    ...
                    const legacyDetail = Array.isArray(existing.details) ?
                        existing.details.find((item) => /browser/i.test((item?.name || item?.label || '')
                            .toLowerCase()) && typeof item?.value === 'string') :
                        null;
                    ...
                    const legacySpec = Array.isArray(existing.specs) ?
                        existing.specs.find((item) => /browser/i.test((item?.name || '').toLowerCase()) &&
                ...
                function addBrowserChip(label) {
                    ...
                    chip.className =
                        'inline-flex items-center gap-1 rounded-full bg-cyan-500/10 px-3 py-1 text-xs font-bold text-cyan-700';
                ...
                function syncJson() {
                    ...
                    const details = Array.from(listDetails?.querySelectorAll('.detail-row') || [])
                        .map(r => {
                            ...
                            const derivedName = alt || (src ? galleryFileName(src).replace(/\.[^.]+$/,
                                '').replace(/[-_]+/g, ' ').trim() : '');
                ...
                (function init() {
                    ...
                    if (listSpecs) {
                        const dashboardSpecs = existing.specs && !Array.isArray(existing.specs) ?
                            existing.specs : {};
                        const dashboardImages = Array.isArray(dashboardSpecs.images) ? dashboardSpecs
                            .images : [];

                        if (dashboardDescriptionInput && typeof dashboardSpecs.description ===
                            'string') {
                    ...
                    browserPresetButtons.forEach((button) => {
                        button.addEventListener('click', () => addBrowserChip(button.dataset
                            .browserPreset || ''));
                    });
