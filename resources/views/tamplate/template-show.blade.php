      ...
      <div x-data="{ addReview: false }" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/50 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 p-6 sm:p-8">
        ...
        <div x-show="addReview" x-collapse class="mb-8" x-data="{ rating: 0, hoverRating: 0 }">
          ...
          @endif
              ...
              @guest('client')
                @guest
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <input type="text" name="author_name" placeholder="اسمك الكامل"
                      class="w-full bg-slate-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-600 rounded-md focus:ring-primary focus:border-primary transition" required>
                    <input type="email" name="author_email" placeholder="بريدك الإلكتروني (لن يتم نشره)"
                      class="w-full bg-slate-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-600 rounded-md focus:ring-primary focus:border-primary transition" required>
                  </div>
                  @endguest
                @endguest
                  <textarea name="comment" placeholder="اكتب مراجعتك هنا..." rows="4"
                  ...
                  <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <button class="bg-primary hover:bg-primary/90 text-white font-bold py-2 px-6 rounded-lg transition-all duration-300 transform hover:-translate-y-0.5">
                      إرسال المراجعة
                    </button>
                  </div>
                </div>
            ...
            <div class="space-y-6">
              @php
              // لتخفيف N+1 (اختياري): يمكنك عمل eager load لهذه العلاقات في الكونترولر:
              // $template->load(['reviews' => fn($q) => $q->approved()->latest()->take(10), 'reviews.client:id,first_name,last_name,avatar', 'reviews.user:id,name']);
              @endphp
                  ...
                  <div class="flex-1">
                    <div class="flex flex-wrap gap-2 justify-between items-center mb-1">
                      ...
                      <div class="flex text-yellow-400" aria-label="تقييم {{ (int)$review->rating }} من 5">
                        @for($i=1;$i<=5;$i++)
                          <svg class="w-4 h-4 {{ $review->rating >= $i ? '' : 'text-gray-300 dark:text-gray-600' }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                          </svg>
                        @endfor
                      </div>
