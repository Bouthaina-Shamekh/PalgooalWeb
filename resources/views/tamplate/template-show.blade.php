        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/50 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 overflow-hidden">
          ...
          <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            @if ($translation?->preview_url)
              <a href="{{ route('template.preview', $translation->slug) }}" target="_blank" class="flex items-center justify-center gap-2.5 text-center bg-primary hover:bg-primary/90 text-white py-3 rounded-lg font-bold shadow-lg shadow-primary/30 hover:shadow-primary/40 transition-all duration-300 transform hover:-translate-y-0.5">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z" /><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.022 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" /></svg>
                <span>{{ t('Frontend.View_Template', 'View Template')}}</span>
              </a>
            @endif
            <a href="#" class="flex items-center justify-center gap-2.5 text-center bg-green-500 hover:bg-green-600 text-white py-3 rounded-lg font-bold shadow-lg shadow-green-500/30 hover:shadow-green-500/40 transition-all duration-300 transform hover:-translate-y-0.5">
          ...
          @if(count($features))
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
              @foreach($features as $feature)
                <div class="bg-slate-50 dark:bg-gray-700/50 p-4 rounded-xl text-center border border-gray-200 dark:border-gray-700 transition-all duration-300 hover:shadow-lg hover:border-primary/50 hover:-translate-y-1">
          ...
          @if ($gallery->isNotEmpty())
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              @foreach ($gallery as $item)
                ...
                @endphp
                <a href="{{ $url }}" class="group block rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow duration-300">
        ...
        <div x-data="{ addReview: false }" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/50 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 p-6 sm:p-8">
          <div class="flex flex-wrap justify-between items-center gap-4 mb-6">
            ...
            <h3 class="flex items-center gap-3 text-xl font-bold text-gray-800 dark:text-white">
              ...
              <span class="text-xs font-normal text-gray-500 dark:text-gray-400 ms-2">({{ $reviewsCount }})</span>
            </h3>
          ...
          <div x-show="addReview" x-collapse class="mb-8" x-data="{ rating: 0, hoverRating: 0 }">
            <form
              ...
              <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                <h4 class="font-bold text-lg text-gray-800 dark:text-white mb-4">{{ t('Frontend.Share_your_opinion_with_us', 'Share your opinion with us') }}</h4>
                @guest('client')
                  @guest
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                      <input type="text" name="author_name" placeholder="{{ t('Frontend.your_name', 'your name') }}" class="w-full bg-slate-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-600 rounded-md focus:ring-primary focus:border-primary transition" required>
                      <input type="email" name="author_email" placeholder="{{ t('Frontend.Your_email_(will_not_be_published)', 'Your email (will not be published)') }}" class="w-full bg-slate-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-600 rounded-md focus:ring-primary focus:border-primary transition" required>
                    </div>
                  @endguest
                @endguest
                <textarea name="comment" placeholder="{{ t('Frontend.Write_your_review_here...', 'Write your review here...') }}" rows="4" class="w-full bg-slate-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-600 rounded-md focus:ring-primary focus:border-primary transition mb-4"></textarea>
                <input type="hidden" name="rating" :value="rating">
                <div class="flex flex-wrap items-center justify-between gap-4">
                  <div class="flex items-center gap-2">
                    <span class="font-semibold text-sm text-gray-700 dark:text-gray-300">{{ t('Frontend.Your_rating:', 'Your rating:') }}</span>
                  ...
                  <button class="bg-primary hover:bg-primary/90 text-white font-bold py-2 px-6 rounded-lg transition-all duration-300 transform hover:-translate-y-0.5">{{ t('Frontend.Submit_review:', 'Submit review') }}</button>
                </div>
              </div>
            </form>
          </div>
          ...
          <div class="space-y-6">
            @foreach ($template->reviews()->approved()->latest()->take(10)->get() as $review)
              @php
                // الاسم المعرُوض
                $displayName = ($review->client?->first_name || $review->client?->last_name)
                  ? trim(($review->client?->first_name ?? '').' '.($review->client?->last_name ?? ''))
                  : ($review->user?->name ?? ($review->author_name ?? 'مستخدم'));
                // الصورة
                $avatarName = $review->author_name ?? $review->client?->first_name ?? $review->user?->name ?? 'User';
              ...
              <div class="flex gap-4">
                <img loading="lazy" src="{{ $avatarUrl }}" alt="صورة العميل" class="w-12 h-12 rounded-full object-cover border-2 border-primary/50">
                  <div class="flex-1">
                    <div class="flex justify-between items-center mb-1">
                      ...
                      @endif
                      <div class="flex text-yellow-400" aria-label="تقييم {{ (int)$review->rating }} من 5">
                        <!-- نجوم التقييم -->
                        @for($i=1;$i<=5;$i++)
                          <svg class="w-4 h-4 {{ $review->rating >= $i ? '' : 'text-gray-300 dark:text-gray-600' }}" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                        @endfor
                      </div>
                    </div>
              ...
              @endforeach
              @if($reviewsCount === 0)
                <p class="text-sm text-gray-500 dark:text-gray-400">لا توجد مراجعات بعد — كن أول من يقيّم القالب.</p>
              @endif
            </div>
          </div>
        </div>
    ...
    money(v){ return '$' + Number(v).toFixed(2); }
          ...
          {{-- <div x-data="{ open: false }" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/50 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 p-6 sm:p-8 flex flex-col gap-5">
            ...
            <div class="flex justify-between items-center">
              <p class="text-base font-medium text-gray-600 dark:text-gray-300">{{ t('Frontend.Price', 'Price') }}:</p>
              <div class="flex items-baseline gap-2">
                <span class="text-4xl font-bold text-primary dark:text-primary-400">${{ number_format($finalPrice, 2) }}</span>
                @if($hasDiscount)
                  <span class="text-xl text-gray-400 dark:text-gray-500 line-through">${{ number_format($template->price, 2) }}</span>
                @endif
              </div>
            </div>
            @if($hasDiscount && $endsAt)
              <div class="bg-red-100/50 dark:bg-red-900/20 text-sm p-3 rounded-lg text-red-700 dark:text-red-300 font-semibold text-center">
                <p>🔥 {{ t('Frontend.Offer_ends_in', 'Offer ends in') }}: <span class="font-mono tracking-wider">24:17:55</span></p>
              </div>
            @endif
            <div class="grid grid-cols-2 gap-x-6 gap-y-4 text-sm pt-2">
              ...
              @empty
                <div class="font-semibold text-gray-800 dark:text-gray-200">{{ t('Frontend.No_specifications', 'No specifications') }}</div>
              @endforelse
              @if($detailsList->count() > 3)
                <template x-if="open">
                  <div class="contents">
