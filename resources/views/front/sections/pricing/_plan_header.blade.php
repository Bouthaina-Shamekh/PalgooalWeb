    ...
    @if ($monthlyPrice !== null)
        <div class="mt-4 pt-4 border-t border-white/20">
            <div class="flex items-end justify-center gap-1">
                ...
                <span class="text-white/70 text-sm mb-1.5">{{ t('site.Per_Month', '/mo') }}</span>
            </div>
            @if ($annualPrice && $yearlySaving)
                <p class="text-[#d9d9d9] text-xs text-center mt-2 transition-opacity duration-300 min-h-[16px]"
                    ...
                    {{ strtr(t('site.Billed_Per_Year', 'Billed $:amount / year'), [':amount' => number_format($annualPrice, 2)]) }}
                    &mdash;
                    <span class="text-white font-semibold">{{ strtr(t('site.Save_Per_Year', 'save $:amount'), [':amount' => $yearlySaving]) }}</span>
                </p>
            @else
