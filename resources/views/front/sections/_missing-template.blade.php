@php
    ...
    $message = trim(
        (string) ($missingTemplate['message'] ?? __('The requested section renderer could not be resolved.')),
    );
    $details = collect(is_array($missingTemplate['details'] ?? null) ? $missingTemplate['details'] : [])
        ->map(static fn($detail) => trim((string) $detail))
        ->filter()
        ->values();
    $attemptedViews = collect(
        is_array($missingTemplate['attempted_views'] ?? null) ? $missingTemplate['attempted_views'] : [],
    )
    ...
    <div class="mx-auto max-w-4xl rounded-3xl border border-rose-200 bg-rose-50 px-6 py-8 text-rose-900">
        ...
        <div class="mt-6 space-y-2 text-xs text-rose-800">
            ...
            @if (!empty($missingTemplate['resolved_section_type']))
                <p><strong>{{ __('Resolved Section Type') }}:</strong>
                    <code>{{ $missingTemplate['resolved_section_type'] }}</code></p>
            @endif

            @if (!empty($missingTemplate['resolution_source']))
                <p><strong>{{ __('Resolution Source') }}:</strong>
                    <code>{{ $missingTemplate['resolution_source'] }}</code></p>
            @endif
