    ...
    <section id="{{ $sectionId }}" class="bg-theme-primary font-theme-body py-16 lg:py-24 px-4 sm:px-6 lg:px-12 text-center overflow-hidden">
        <div class="container mx-auto">
            ...
            @if ($title !== '' || $subtitle !== '')
                <div class="mb-16">
                    @if ($title !== '')
                        <h2 class="font-theme-heading text-theme-surface text-2xl md:text-3xl font-extrabold mb-2">
                            {{ $title }}
                        </h2>
                    @endif

                    @if ($subtitle !== '')
                        <p class="text-theme-surface text-lg md:text-xl font-light opacity-90">
                            {{ $subtitle }}
                        </p>
                    @endif
                </div>
            ...
            @if ($steps->isNotEmpty())
                <div class="grid grid-cols-1 {{ $gridClass }} gap-12 md:gap-10 relative max-w-7xl mx-auto">
                    @foreach ($steps as $step)
                        <div @class([
                            'relative rounded-theme-lg p-6 flex flex-col items-center justify-center gap-3 min-h-30 shadow-theme transform transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl',
                            'bg-theme-secondary text-theme-surface z-20' => $step['is_featured'],
                            'bg-theme-surface text-theme-heading z-10' => ! $step['is_featured'],
                        ])>
                            @if ($step['icon_source'] === 'media' && $step['icon_media_url'])
                                <img
                                    src="{{ $step['icon_media_url'] }}"
                                    alt=""
                            ...
                            @elseif ($step['icon'] !== '')
                                <i @class([
                                    $step['icon'],
                                    'text-2xl',
                                    'text-theme-surface' => $step['is_featured'],
                                    'text-theme-secondary' => ! $step['is_featured'],
                                ]) aria-hidden="true"></i>
                            @else
                                <span @class([
                                    'h-3 w-3 rounded-full',
                                    'bg-theme-surface' => $step['is_featured'],
                                    'bg-theme-secondary' => ! $step['is_featured'],
