@php
    ...
    $socialIconClass = trim($theme['social_icon'] . ' opacity-70 hover:opacity-100 hover:scale-110 transition-all duration-300');
    $promoActionClass = trim('flex items-center gap-2 transition-all duration-300 text-sm md:text-base ' . $theme['promo_hover']);
    $languageMenuClass = trim(
        'absolute top-full ltr:right-0 rtl:left-0 mt-2 min-w-[140px] border rounded-xl shadow-lg z-[100] overflow-hidden ' .
        ...
        <div class="container mx-auto flex flex-wrap justify-around md:justify-between items-center gap-3 md:gap-0">
            ...
            @if ($showSocialIcons)
                ...
                <div class="flex items-center gap-4 md:gap-6">
                    <!-- Facebook -->
                    <a href="{{ $safeUrl($socialLinks['facebook'] ?? '#') }}"
                    ...
                    <a href="{{ $safeUrl($socialLinks['whatsapp'] ?? '#') }}"
                    ...
                    <a href="{{ $safeUrl($socialLinks['linkedin'] ?? '#') }}"
                    ...
                    <a href="{{ $safeUrl($socialLinks['twitter'] ?? '#') }}"
                    ...
                    <a href="{{ $safeUrl($socialLinks['instagram'] ?? '#') }}"
            ...
            <div class="flex items-center gap-4 md:gap-6">
                @if ($shouldRenderLoginButton)
                    <a href="{{ $safeUrl($resolvedLoginUrl) }}"
                        class="{{ $promoActionClass }}">
                ...
                @if ($showLanguageSwitcher)
                    <x-lang.language-switcher variant="topbar" :label="$languageSwitcherLabel"
                        :show-locale-code="true" :button-class="$promoActionClass"
...
<!-- Navigation -->
<nav
    class="border-b px-4 sm:px-6 lg:px-12 py-4 {{ $theme['nav_shell'] }} {{ $headerIsSticky ? 'sticky top-0 z-[70] lg:z-50' : 'relative z-[70] lg:z-30' }}">
    <div class="container mx-auto flex justify-between items-center gap-4">
        ...
        <div class="hidden lg:flex justify-between items-center gap-8 lg:gap-10 font-bold {{ $theme['nav_text'] }}">
            @forelse ($header?->items ?? [] as $item)
                @if ($item->type === 'link' || $item->type === 'page')
                    <a href="{{ $safeUrl($item->url) }}" class="transition-colors {{ $theme['nav_hover'] }}">
                        {{ $item->label }}
                    </a>
                @elseif ($item->type === 'dropdown')
                    @php
                        $dropdownId = 'purple-topbar-dropdown-' . $item->id;
                    ...
                    @if (!empty($dropdownChildren))
                        <div class="relative group">
                            ...
                            <div
                                ...
                                @foreach ($dropdownChildren as $child)
        ...
        <a href="{{ $safeUrl($contactButtonUrl) }}"
        ...
        <button id="mobile-menu-toggle"
            type="button"
            class="lg:hidden flex flex-col gap-1.5 p-2 z-[75] relative"
            aria-label="{{ t('frontend.Toggle_Mobile_Menu', 'Toggle mobile menu') }}"
            aria-expanded="false"
        ...
        <div id="mobile-menu"
            ...
            <div id="mobile-menu-container"
                ...
                aria-modal="true"
                aria-hidden="true"
                aria-label="{{ t('frontend.Mobile_Menu', 'Mobile menu') }}"
                tabindex="-1">
