@php
    ...
    $announcementText = trim((string) ($variantSettings['announcement_text'] ?? 'Launch your own website in 5 minutes at minimal cost'));
    $showSocialIcons = (bool) ($variantSettings['show_social_icons'] ?? true);
    $showLoginButton = (bool) ($variantSettings['show_login_button'] ?? true);
    $loginLabel = trim((string) ($variantSettings['login_label'] ?? 'Login'));
    ...
    $languageLabel = trim((string) ($variantSettings['language_label'] ?? 'Language'));
    $contactButtonLabel = trim((string) ($variantSettings['contact_button_label'] ?? 'Contact us'));
...
<style>
    .bg-purple-brand { background-color: #240A37; }
    .text-red-brand { color: #BA112C; }
    .border-red-brand { border-color: #BA112C; }
    .fill-red-brand { fill: #BA112C; }
    .fill-\[\#7F6F8A\] { fill: #7F6F8A; }
    .hover\:text-red-brand:hover { color: #BA112C; }
    .hover\:fill-red-brand:hover { fill: #BA112C; }
    .hover\:bg-red-brand:hover { background-color: #BA112C; }
    .hover\:border-red-brand:hover { border-color: #BA112C; }
</style>

@if ($showPromoBar)
<header class="bg-purple-brand text-white py-3 px-4 sm:px-6 lg:px-12">
    <div class="container mx-auto flex flex-wrap justify-around md:justify-between items-center gap-3 md:gap-0">
        ...
        @if ($showSocialIcons)
            ...
            <a href="{{ $socialLinks['facebook'] ?? '#' }}"
                class="fill-[#7F6F8A] hover:fill-red-brand opacity-70 hover:opacity-100 hover:scale-110 transition-all duration-300">
        ...
        <div class="flex items-center gap-4 md:gap-6">
            @if ($showLoginButton)
                <a href="{{ $loginUrl }}"
                    class="flex items-center gap-2 hover:text-red-brand transition-all duration-300 text-sm md:text-base">
...
<nav class="bg-white border-b border-gray-100 px-4 sm:px-6 lg:px-12 py-4 {{ $headerIsSticky ? 'sticky top-0 z-50' : 'relative z-30' }}">
    <div class="container mx-auto flex justify-between items-center gap-4">
        ...
        <div class="hidden lg:flex justify-between items-center gap-8 lg:gap-10 font-bold text-black">
            <a href="#hosting" class="hover:text-red-brand transition-colors">Hosting</a>
            <a href="#programming" class="hover:text-red-brand transition-colors">Programming</a>
            <a href="#design" class="hover:text-red-brand transition-colors">Design</a>
            <a href="#work" class="hover:text-red-brand transition-colors">Our work</a>
            <a href="#templates" class="hover:text-red-brand transition-colors">Templates</a>
            <a href="#price" class="hover:text-red-brand transition-colors">Price</a>
        </div>

        <a href="{{ $contactButtonUrl }}"
            class="hidden lg:block text-red-brand border border-red-brand px-4 py-2 rounded-xl hover:bg-red-brand hover:text-white transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            {{ $contactButtonLabel }}
        </a>

        <button id="mobile-menu-toggle" class="lg:hidden flex flex-col gap-1.5 p-2 z-[60] relative" aria-label="Open menu">
            <span class="w-6 h-0.5 bg-purple-brand transition-all duration-300 origin-center"></span>
            <span class="w-6 h-0.5 bg-purple-brand transition-all duration-300"></span>
            <span class="w-6 h-0.5 bg-purple-brand transition-all duration-300 origin-center"></span>
        </button>
        ...
        <div id="mobile-menu"
            ...
            <div id="mobile-menu-container"
                class="absolute top-0 ltr:right-0 rtl:left-0 w-[80%] max-w-sm h-full bg-white shadow-2xl ltr:translate-x-full rtl:-translate-x-full transition-transform duration-500 ease-out p-8 flex flex-col pt-24">

                <div class="flex flex-col gap-8 font-bold text-xl text-black">
                    <a href="/" class="hover:text-red-brand transition-colors w-full border-b border-gray-100 pb-4">Home</a>
                    <a href="#hosting" class="hover:text-red-brand transition-colors w-full border-b border-gray-100 pb-4">Hosting</a>
                    <a href="#programming" class="hover:text-red-brand transition-colors w-full border-b border-gray-100 pb-4">Programming</a>
                    <a href="#design" class="hover:text-red-brand transition-colors w-full border-b border-gray-100 pb-4">Design</a>
                    <a href="#work" class="hover:text-red-brand transition-colors w-full border-b border-gray-100 pb-4">Our work</a>
                    <a href="#templates" class="hover:text-red-brand transition-colors w-full border-b border-gray-100 pb-4">Templates</a>
                    <a href="#price" class="hover:text-red-brand transition-colors w-full border-b border-gray-100 pb-4">Price</a>
                </div>

                <div class="mt-auto space-y-6">
                    <a href="{{ $contactButtonUrl }}"
                        class="block w-full text-center text-red-brand border-2 border-red-brand px-6 py-4 rounded-2xl font-bold text-xl hover:bg-red-brand hover:text-white transition-all shadow-lg">
                        {{ $contactButtonLabel }}
                    </a>
                    <div class="flex justify-center items-center">
                        <img src="{{ $logoSrc }}" alt="{{ $settings?->site_title ?? 'Palgoals' }}" class="h-12 w-auto">
                    </div>
                </div>
            </div>
        </div>
