@php
    ...
    $resolveLocalizedText = static function ($value, string $default = '') use (
        $currentLocale,
        $fallbackLocale,
    ): string {
        if (is_array($value)) {
            ...
            $localizedValue = trim(
                (string) ($normalizedValues[$currentLocale] ?? ($normalizedValues[$fallbackLocale] ?? '')),
            );
    ...
    foreach ($configuredThemes as $themeKey => $themeConfig) {
        ...
        $themeClasses = is_array($themeConfig['classes'] ?? null) ? $themeConfig['classes'] : $themeConfig;
    ...
    $defaultThemeKey = strtolower(
        trim((string) config('front_layouts.color_libraries.purple_topbar.default', 'classic')),
    );
    ...
    $languageSwitcherLabel = t('frontend.Language', 'Language');
    $socialIconClass = trim(
        $theme['social_icon'] . ' opacity-70 hover:opacity-100 hover:scale-110 transition-all duration-300',
    );
    $promoActionClass = trim(
        'flex items-center gap-2 transition-all duration-300 text-sm md:text-base ' . $theme['promo_hover'],
    );
    $languageMenuClass = trim(
        'absolute top-full ltr:right-0 rtl:left-0 mt-2 min-w-[140px] border rounded-xl shadow-lg z-[75] overflow-hidden ' .
            $theme['dropdown_shell'],
    );
    $languageItemClass = trim(
        'block w-full ltr:text-left rtl:text-right px-4 py-3 transition-colors text-sm font-medium ' .
            $theme['dropdown_item'],
    );
    ...
    $logoAlt = $settings?->resolved_site_title ?? 'Palgoals';
    $safeUrl = static fn($value): string => SafeUrl::toHref($value);
    $safeLoginUrl = $safeUrl($resolvedLoginUrl);
    $safeContactButtonUrl = $safeUrl($contactButtonUrl);
    $normalizeDropdownChildren = static function ($children) use ($safeUrl): array {
        ...
        foreach ($children as $child) {
            $label = trim((string) ($child['current_label'] ?? ''));
            $url = $safeUrl($child['current_url'] ?? '#');
    ...
    foreach ($header?->items ?? [] as $item) {
        ...
        if ($item->type === 'dropdown') {
            $dropdownChildren = $normalizeDropdownChildren($item->processedChildren ?? []);

            if (empty($dropdownChildren)) {
                continue;
            }

            $navigationItems[] = [
                'type' => 'dropdown',
                'id' => 'purple-topbar-dropdown-' . $item->id,
                'label' => $label,
...
@if ($showPromoBar)
    <header class="relative z-[71] py-3 px-4 sm:px-6 lg:px-12 {{ $theme['promo_bar'] }}">
        <div class="container mx-auto flex flex-wrap justify-around md:justify-between items-center gap-3 md:gap-0">
            ...
            @if ($showSocialIcons && !empty($socialItems))
                <!-- Social Media Icons -->
                <div class="flex items-center gap-4 md:gap-6">
                    @foreach ($socialItems as $socialItem)
                        <a href="{{ $socialItem['href'] }}" aria-label="{{ $socialItem['label'] }}"
                            class="{{ $socialIconClass }}">
                            <svg class="h-4" viewBox="{{ $socialItem['viewBox'] }}" xmlns="http://www.w3.org/2000/svg"
                                aria-hidden="true" focusable="false">
                                <path d="{{ $socialItem['path'] }}" />
                            </svg>
                        </a>
                    @endforeach
                </div>
            ...
            <div class="flex items-center gap-4 md:gap-6">
                @if ($shouldRenderLoginButton)
                    <a href="{{ $safeLoginUrl }}" class="{{ $promoActionClass }}">
                ...
                @if ($showLanguageSwitcher)
                    <x-lang.language-switcher variant="topbar" :label="$languageSwitcherLabel" :show-locale-code="true" :button-class="$promoActionClass"
                        :menu-class="$languageMenuClass" :item-class="$languageItemClass" />
                @endif
            </div>
        </div>
    </header>
@endif
<!-- Navigation -->
<div id="main-header" data-sticky-header class="relative z-[70] lg:z-50">
    <nav data-sticky-header-nav
        class="w-full border-b px-4 sm:px-6 lg:px-12 py-4 transition-shadow duration-300 {{ $theme['nav_shell'] }}">
        <div class="container mx-auto flex justify-between items-center gap-4">
            <!-- Logo -->
            <div class="flex items-center gap-2 ltr:flex-row rtl:flex-row-reverse">
                <img src="{{ $logoSrc }}" alt="{{ $logoAlt }}" class="h-14 w-auto">
            </div>
            <!-- Desktop Navigation -->
            <div
                class="hidden lg:flex justify-between items-center gap-8 lg:gap-10 font-bold {{ $theme['nav_text'] }}">
                @foreach ($navigationItems as $item)
                    ...
                    @elseif ($item['type'] === 'dropdown')
                        <div class="relative group">
                            <button type="button" id="{{ $item['id'] }}-toggle"
                                class="flex items-center gap-1 cursor-pointer transition-colors bg-transparent border-0 p-0 font-bold {{ $theme['nav_hover'] }}"
                                aria-haspopup="true" aria-expanded="false" aria-controls="{{ $item['id'] }}"
                            ...
                            <div id="{{ $item['id'] }}"
                                class="absolute top-full end-0 mt-2 w-56 border rounded-lg shadow-md z-50 text-sm font-normal opacity-0 invisible scale-95 group-hover:opacity-100 group-hover:visible group-hover:scale-100 group-focus-within:opacity-100 group-focus-within:visible group-focus-within:scale-100 transition-all duration-200 {{ $theme['dropdown_shell'] }}">
            ...
            <!-- Mobile Menu Toggle -->
            <button id="mobile-menu-toggle" type="button" class="lg:hidden flex flex-col gap-1.5 p-2 z-[75] relative"
                aria-label="{{ t('frontend.Toggle_Mobile_Menu', 'Toggle mobile menu') }}" aria-expanded="false"
            ...
            <!-- Mobile Menu Overlay -->
            <div id="mobile-menu"
                class="fixed inset-0 bg-black/60 backdrop-blur-md z-[80] invisible opacity-0 transition-all duration-500 lg:hidden">
                <div id="mobile-menu-container"
                    class="fixed inset-y-0 ltr:right-0 rtl:left-0 w-[80%] max-w-sm overflow-y-auto shadow-2xl ltr:translate-x-full rtl:-translate-x-full transition-transform duration-500 ease-out p-8 flex flex-col pt-24 {{ $theme['mobile_panel'] }}"
                    role="dialog" aria-modal="true" aria-hidden="true"
                    aria-label="{{ t('frontend.Mobile_Menu', 'Mobile menu') }}" tabindex="-1">
                        ...
                        @foreach ($navigationItems as $item)
                            ...
                            @elseif ($item['type'] === 'dropdown')
                                <div class="w-full border-b pb-4 {{ $theme['mobile_link_border'] }}">
                                    ...
                                    <div
                                        class="mt-3 flex flex-col gap-3 pe-4 text-base font-semibold {{ $theme['mobile_subtext'] }}">
                                        @foreach ($item['children'] as $child)
                                            <a href="{{ $child['current_url'] }}"
