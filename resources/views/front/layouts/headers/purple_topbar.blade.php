@php
    ...
    $announcementText = trim(
        (string) ($variantSettings['announcement_text'] ?? 'Launch your own website in 5 minutes at minimal cost'),
    );
...
@if ($showPromoBar)
    <header class="relative z-[60] bg-purple-brand text-white py-3 px-4 sm:px-6 lg:px-12">
...
<!-- Navigation -->
<nav
    class="bg-white border-b border-gray-100 px-4 sm:px-6 lg:px-12  py-4 {{ $headerIsSticky ? 'sticky top-0 z-50' : 'relative z-30' }}">
    <div class="container mx-auto flex justify-between items-center gap-4">
        ...
        <div class="hidden lg:flex justify-between items-center gap-8 lg:gap-10 font-bold text-black">
            @forelse ($header?->items ?? [] as $item)
                @if ($item->type === 'link' || $item->type === 'page')
                    <a href="{{ $item->url }}" class="hover:text-red-brand transition-colors">
                        {{ $item->label }}
                    </a>
                @elseif ($item->type === 'dropdown')
                    <div class="relative group">
                        <div class="flex items-center gap-1 cursor-pointer hover:text-red-brand transition-colors">
                        ...
                        <div
                            class="absolute top-full end-0 mt-2 w-56 bg-white border border-gray-200 rounded-lg shadow-md z-50 text-sm font-normal opacity-0 invisible scale-95 group-hover:opacity-100 group-hover:visible group-hover:scale-100 transition-all duration-200">
                            @foreach ($item->processedChildren as $child)
                                <a href="{{ $child['current_url'] ?? '#' }}"
                                    class="block px-4 py-2 text-black hover:bg-gray-100 hover:text-red-brand transition-colors">
                                    {{ $child['current_label'] ?? '-' }}
                                </a>
                            @endforeach
                        </div>
        ...
        <a href="{{ $contactButtonUrl }}"
            class="hidden lg:block text-purple-brand border border-red-brand px-4 py-2 rounded-xl hover:bg-red-brand hover:text-white transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            {{ $contactButtonLabel }}
        </a>
        <!-- Mobile Menu Toggle -->
        ...
        <div id="mobile-menu"
            ...
            <div id="mobile-menu-container"
                ...
                <div class="flex flex-col gap-6 font-bold text-xl text-black">
                    @forelse ($header?->items ?? [] as $item)
                        @if ($item->type === 'link' || $item->type === 'page')
                            <a href="{{ $item->url }}"
                                class="hover:text-red-brand transition-colors w-full border-b border-gray-100 pb-4">
                                {{ $item->label }}
                            </a>
                        @elseif ($item->type === 'dropdown')
                            <div class="w-full border-b border-gray-100 pb-4">
                                ...
                                @if (!empty($item->processedChildren))
                                    <div class="mt-3 flex flex-col gap-3 pe-4 text-base font-semibold text-gray-dark">
                                        @foreach ($item->processedChildren as $child)
                                            <a href="{{ $child['current_url'] ?? '#' }}"
                                                class="hover:text-red-brand transition-colors">
                                                {{ $child['current_label'] ?? '-' }}
                                            </a>
                                        @endforeach
                                    </div>
                ...
                <div class="mt-auto space-y-6">
                    <a href="{{ $contactButtonUrl }}"
                        class="block w-full text-center text-purple-brand border-2 border-red-brand px-6 py-4 rounded-2xl font-bold text-xl hover:bg-red-brand hover:text-white transition-all shadow-lg">
                        {{ $contactButtonLabel }}
                    </a>
