        ...
        @forelse ($header->items ?? [] as $item)
            ...
            @elseif ($item->type === 'dropdown')
                <li class="relative group">
                    <div
                        class="flex items-center gap-1 cursor-pointer hover:text-secondary dark:hover:text-yellow-400 transition">
                        <span>{{ $item->label }}</span>
                        <svg class="w-4 h-4 mt-0.5 transform transition-transform group-hover:rotate-180" fill="none"
                            stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                        </svg>
                    </div>
                    <ul
                        class="absolute top-full end-0 mt-2 w-48 bg-white dark:bg-[#2c2c2c] border border-gray-200 dark:border-gray-700
